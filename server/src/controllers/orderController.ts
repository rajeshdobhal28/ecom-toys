...
export const createOrder = async (req: AuthRequest, res: Response): Promise<void> => {
    try {
        ...
        if (!products || !Array.isArray(products) || products.length === 0) {
            res.status(400).send({ status: 'error', message: 'Products array is required and must not be empty' });
            return;
        }
        ...
        for (const item of products) {
            if (!item.productId || !item.quantity) {
                res.status(400).send({ status: 'error', message: 'Each product must have a productId and quantity' });
                return;
            }
        }
